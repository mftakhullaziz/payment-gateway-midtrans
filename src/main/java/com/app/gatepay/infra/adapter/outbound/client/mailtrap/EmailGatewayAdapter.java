package com.app.gatepay.infra.adapter.outbound.client.mailtrap;

import com.app.gatepay.domain.email.EmailGatewayPort;
import com.app.gatepay.shared.exception.NotifyEmailException;
import com.app.gatepay.shared.resources.EmailResource;
import jakarta.mail.*;
...
public class EmailGatewayAdapter implements EmailGatewayPort {
    ...
    private void sendMailExecutor(String htmlContent, String email, String subject) {
        try {
            ...
            message.setFrom(new InternetAddress(emailResource.getMailDev().getSender()));
            ...
            throw new NotifyEmailException("Unexpected server error: " + e.getMessage());
        }
    }

    private Session createSession() {
        Properties props = new Properties();
        props.put("mail.smtp.host", emailResource.getMailDev().getHostname()); // smtp.mailtrap.io
        props.put("mail.smtp.port", emailResource.getMailDev().getPort());     // 587
        ...
        Authenticator auth = new Authenticator() {
            ...
            protected PasswordAuthentication getPasswordAuthentication() {
                return new PasswordAuthentication(
                    emailResource.getMailDev().getUsername(),
                    emailResource.getMailDev().getPassword()
                );
            }
        };
    ...
    private String loadTemplate(String templateName) {
        try (InputStream inputStream = getClass()
            ...
            throw new NotifyEmailException("Failed to load email template: " + e.getMessage());
        }
    }
